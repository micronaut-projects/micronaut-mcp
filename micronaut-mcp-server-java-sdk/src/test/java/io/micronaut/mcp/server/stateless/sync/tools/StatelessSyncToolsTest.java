...
class StatelessSyncToolsTest {
    ...
    void toolsList(@Client("/") HttpClient httpClient) throws JSONException {
        ...
        HttpResponse<String> resp = assertDoesNotThrow(() -> client.exchange(req, String.class));
        assertEquals(MediaType.APPLICATION_JSON_TYPE, resp.getContentType().get());
        String result = resp.getBody(String.class).get();
